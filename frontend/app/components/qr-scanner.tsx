    ...
    <div className="space-y-4">
      <div className="relative overflow-hidden rounded-2xl bg-slate-950 shadow-inner">
        ...
        {(!scanning || status || error || disabled) && (
          <div className="absolute inset-0 grid place-items-center bg-slate-950/85 p-6 text-center text-white">
            <div>
              <div className="mx-auto mb-3 grid h-12 w-12 place-items-center rounded-2xl bg-white/10 text-2xl">▣</div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        <button
          ...
          className={`rounded-xl px-4 py-3 text-sm font-bold text-white transition disabled:cursor-not-allowed disabled:bg-slate-300 ${
        ...
        {cameras.length > 1 && (
          <select
            ...
            disabled={disabled}
            className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-3 py-3 text-sm text-slate-700 outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-100"
            aria-label="Camera"
          >
