  ...
  return (
    <main className="mx-auto grid w-full max-w-7xl gap-6 px-4 py-6 sm:px-6 lg:grid-cols-[minmax(0,0.82fr)_minmax(0,1.18fr)] lg:px-8 lg:py-8">
      <section className="panel p-5 sm:p-6">
        ...
        <div className="mt-6 rounded-2xl bg-amber-50 p-4 text-sm leading-6 text-amber-800"><strong>Safety rule:</strong> attended events cannot be replaced, and a ticket cannot contain the same event twice.</div>
      </section>
      ...
      <section className="panel min-h-[32rem] overflow-hidden">
        ...
        {catalogError && <div className="mx-5 mt-5 rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700 sm:mx-6" role="alert">{catalogError}</div>}

        {!data && <div className="grid place-items-center px-6 py-24 text-center"><div><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-amber-50 text-2xl text-amber-600">↔</div><p className="mt-4 font-bold text-slate-700">No ticket loaded</p><p className="mt-1 text-sm text-slate-500">Scan a ticket to see its current event assignments.</p></div></div>}
        {data && (
          <>
            ...
            <div className="space-y-3 p-5 sm:p-6">
              ...
              {data.events.map((event) => {
                ...
                return (
                  <article key={event.event_id} className="rounded-2xl border border-slate-200 bg-slate-50/70 p-4 sm:p-5">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                      <div className="min-w-0">
                        <div className="flex flex-wrap items-center gap-2"><span className="rounded-lg bg-white px-2.5 py-1 text-xs font-bold text-slate-500 shadow-sm">Event {event.position}</span>{event.attendance && <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">Attended · locked</span>}</div>
                      ...
                      {!event.attendance && (
                        <div className="flex min-w-0 flex-col gap-2 sm:flex-row lg:w-[28rem]">
                          ...
                          <button type="button" onClick={() => void handleSave(event)} disabled={!selected || savingId !== null} className="rounded-xl bg-amber-500 px-4 py-3 text-sm font-bold text-white transition hover:bg-amber-600 disabled:cursor-not-allowed disabled:bg-slate-300">{savingId === event.event_id ? "Saving…" : "Save"}</button>
                        </div>
                      )}
                    </div>
                  </article>
