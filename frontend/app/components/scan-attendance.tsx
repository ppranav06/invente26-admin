...
function EventCard({ event, busy, onMark, canMark }: { event: TicketEvent; busy: boolean; onMark: () => void; canMark: boolean }) {
 ...
 <article className=" border border-slate-200 bg-slate-50/70 p-4 transition hover:border-indigo-200 hover:bg-white sm:p-5">
  ...
  <div className="min-w-0">
   ...
   <span className=" bg-white px-2.5 py-1 text-xs font-bold text-slate-500 shadow-sm">Event {event.position}</span>
  ...
  {event.attendance ? (
   <div className="flex shrink-0 items-center gap-2 bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-700">
  ...
  {!loading && data && (
   ...
   {data.hackathon_teams.length > 0 && (
    ...
    <div className="mt-3 grid gap-3 sm:grid-cols-2">
     {data.hackathon_teams.flatMap((team) => team.members).map((member) => <div key={member.member_id} className=" bg-slate-50 px-3 py-2 text-sm"><p className="font-semibold text-slate-800">{member.name}{member.is_lead ? " · Lead" : ""}</p><p className="text-xs text-slate-500">{member.email}</p></div>)}
    </div>
    </div>
   )}
   </>
